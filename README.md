...
最新版に更新します
  ...
  tikhub  6 ファイルを更新  (main)
      src/bridge.js
      src/index.js
      src/live.js
      …
  kawaiivsbeautiful  最新でした  (main)
  circlebattle       8 ファイルを更新  (claude-tiktok-live-battle-game-6tvo3w)
      js/config.js
      js/renderer.js
      …
...
括弧の中は、**実際に入れたブランチ**です。頼んだ名前ではなく返ってきた名前を
出しています (GitHub は `master` を既定ブランチへ黙って読み替えるので、
頼んだ名前をそのまま出すと嘘になることがあります)。
ここを見れば、古いブランチで上書きしてしまったときにすぐ気づけます。
...
- ゲームのフォルダは自動で探します。**複数あれば全部更新します**ので、
  ...
  上書きすることもありません。
- ゲームをまだ 1 つも持っていないときは、**`games/` フォルダを作って**
  取得先の URL を出します。ZIP を展開してそこに入れれば、次からは自動で更新されます。
- 置いた場所が違うときや 1 つだけ更新したいときは
  `npm run update -- --game="ゲームのフォルダ"` で指定してください。

### どのブランチから入れるか

指定しなければ、リポジトリごとの**既定ブランチ**から入れます
(取れなければ `main` → `master` の順に試します)。

ブランチを選びたいときは:

```bash
npm run update -- --branch=claude/tiktok-live-battle-game-6tvo3w
```

`.env` に `GITHUB_BRANCH=...` と書いても同じです。
**指定したときは、そのブランチだけを試します。** 指定したのに別のブランチが
入ってしまうのが一番困るためです。無ければエラーになります。

> 開発中の内容が `main` に取り込まれていないと、`npm run update` は
> 古い `main` で新しいファイルを上書きしてしまいます。表示される
> ブランチ名がいつもと違うときは、`--branch=` で指定してください。
...
**配信を始める前に URL を入れておけます。** その時点では配信していなくてもエラーにはならず、
15 秒ごとに確認して、配信が始まった瞬間に自動で繋がります。

```
@username はまだ配信していません。開始を待ちます (15 秒ごとに確認)
```

画面には「配信の開始を待っています」と出て、繋がると消えます。
別の配信に切り替えたいときは、その画面の「別の配信を指定する」を押してください。

### ゲームのフォルダの置き方

**tikhub の中の `games/` に入れるのがいちばん簡単です。** 全部が 1 つのフォルダに
まとまるので、どこに置いたか分からなくなりません。

```
tikhub-.../                    ← ここで npm start
  ├── src/
  ├── games/                   ← 無ければ作ってください
  │     ├── circlebattle/      ← 自動で見つかる
  │     └── kawaiivsbeautiful/ ← これも自動で見つかる
  └── .env
```

`games/` は tikhub を更新しても**中身をそのまま残します**。
...
どちらでも `index.html` と `js/game.js` が入っていれば見つけます
(ZIP を展開してできる `circlebattle-main/circlebattle-main/` のような
二重フォルダもそのままで大丈夫です)。

見つからないときは起動時のメッセージが置き場所を教えます。
`--game=フォルダ` で直接指定もできます。
