...
**TikTok には、LIVE 配信中のギフト / いいね / フォロー / コメントをリアルタイムに取得できる公式 API は存在しない。**
...
**→ 公式ルートだけでは今回の要件 (ギフト / いいね / フォロー / コメントのリアルタイム受信) は実現できない。**
...
- 接続先の解決 (`src/target.js`) はライブラリから独立させ、ネットワーク不要な部分
  (ユーザー名 / プロフィール URL の解釈) と必要な部分 (短縮 URL の展開) を分けた。
  実接続ができない環境でも、この層だけはテストで検証できる。
